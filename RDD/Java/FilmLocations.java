...
public class FilmLocations {
    public static void main(String[] args) throws IOException {
        ...
        var output = filtered.sortByKey(false).collect();

        double numero_peliculas = groupedPairs.count();

        double numero_local = pares.count();

        double media = numero_local/numero_peliculas;

        PrintWriter myWriter = new PrintWriter("data/output.txt");

        for(Tuple2<?, ?> tuple : output){

            System.out.println(tuple._1() + ": " + tuple._2());
            myWriter.println(tuple._1() + ": " + tuple._2());
        }

        myWriter.println("Total number of films: " + numero_peliculas);
        myWriter.println("The average of film locations per film: " + media);
        myWriter.close();
